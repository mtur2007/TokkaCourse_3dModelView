...
async function setupHuntScenario({ mapRoot, player, camera, objectA, objectB }) {
  ...
  function bStopChase() {
    bChasing = false;
    if (bMoveSound && bMoveSound.isPlaying) bMoveSound.pause();
  }
  ...
  function updateBChase(dt) {
    ...
    if (bMoveSound) {
      if (soundArmed) {
        if (!bMoveSound.isPlaying && bMoveSound.buffer) bMoveSound.play();
      } else if (bMoveSound.isPlaying) {
        bMoveSound.pause();
      }
    }
...
(async () => {
  ...
  // ---- B の移動音（Bが追尾中のみ鳴らす）
  bMoveSound = new THREE.PositionalAudio(listener);
  audioLoader.load("sound/horror.mp3", (buffer) => {
    bMoveSound.setBuffer(buffer);
    bMoveSound.setLoop(true);
    bMoveSound.setVolume(0.6);
    bMoveSound.setRefDistance(2);
  });
  objectB.add(bMoveSound);
